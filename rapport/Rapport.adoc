...
* *Sobriété Numérique :* 
** _Justification_ : L'algorithme a une complexité linéaire O(n), ce qui signifie que le temps d'exécution augmente de manière linéaire avec la taille de l'entrée. +
Cela indique une efficacité raisonnable et une consommation de ressources relativement stable, sans dépendance excessive à la taille de l'entrée.
** _Exemple_ : L'algorithme traite chaque caractère de la chaîne de caractères une seule fois, en les ajoutant aux mots appropriés. La complexité linéaire garantit une consommation de ressources proportionnelle à la taille de la chaîne de caractères donnée.
{emplty} +
